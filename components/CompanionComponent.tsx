...
const CompanionComponent = ({
    ...
    const bgColor = subjectsColors[subject?.toLowerCase()] || "#000";
        ...
        <section className="flex flex-col h-[70vh]">
            <section className="flex gap-8 max-md:flex-col">
                <div className="companion-section">
                    <div className="companion-avatar relative" style={{ backgroundColor: bgColor }}>
                        <div
                            className={cn(
                ...
                <div className="user-section">
                    ...
                    <button className={cn(
                        'rounded-lg py-2 cursor-pointer transition-colors w-full text-white', callStatus === CallStatus.CONNECTING ? 'bg-red-700' : 'bg-primary', callStatus === CallStatus.CONNECTING && 'animate-pulse',
                    )} onClick={callStatus === CallStatus.ACTIVE ? handleDisconnect : handleCall}>
                        {callStatus === CallStatus.CONNECTING ? 'End the Session' : callStatus === CallStatus.CONNECTING  ? "Connecting" : 'Start the Session'}
                    </button>
                </div>
            </section>
            <section className="transcript">
                <div className="transcript-message no-scrollbar">
                    {messages.map((message, index) => (
                        <p key={index} className={message.role === 'assistant' ? '' : 'text-primary'}>
